...
export function allocate_and_write_voxel_material_address_in_memory_of_voxel_octree_at_position(
  ...
  while (voxelOctreeDepth >= 0) {
    ...
    const voxelOctreeChildAddressAddress: IMemoryAddress =
    ...
    if (voxelOctreeDepth === 0) {
      ...
      write_u32_be_in_memory(memory, voxelOctreeChildAddressAddress, voxelMaterialAddress);
      break;
    } else {
      ...
      if (
        ...
        if (voxelOctreeChildAddress === voxelMaterialAddress) {
          ...
          write_u32_be_in_memory(memory, voxelOctreeChildAddressAddress, newVoxelOctreeAddress);
