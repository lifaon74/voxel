 ...
 */
fn get_entry_point_3d_of_ray_3d_with_cube(
  // RAY
  rayStartPoint: vec3<f32>,
  rayEndPoint: vec3<f32>,
  // CUBE
  side: u32,
  // OUTPUTS
  outputRayHitPoint: vec3<f32>,
  outputNormalVector: vec3<f32>,
) -> bool {
  var a: u8;
  var b: u8;
  var c: u8;

  const rayVector: vec3<f32> = rayEndPoint - rayStartPoint;
  ...
  for (a = 0; a < 3; a++) {
    ...
    if (rayVector[a] != 0) {
      ...
      outputRayHitPoint[b] =
        rayStartPoint[b] + (outputRayHitPoint[a] - rayStartPoint[a]) * (rayVector[b] / rayVector[a]); // thales
      ...
      if (
        ...
        outputRayHitPoint[c] =
          rayStartPoint[c] + (outputRayHitPoint[a] - rayStartPoint[a]) * (rayVector[c] / rayVector[a]); // thales
        ...
        if (
          ...
          outputNormalVector[a] = select(-1, 1, rayVector[a] > 0);
